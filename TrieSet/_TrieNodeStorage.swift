...
internal class _TrieNodeStorage<E: TriePrefixing> {
    ...
    init(prefix: TriePrefix?) {
        self.prefix = prefix
        elements = []
        nodesMap = [:]
    }
    
    init(storage: _TrieNodeStorage) {
        ...
        if !prefixes.isEmpty {
            ...
            if var nodes = nodesMap[hashValue] {
                if let index = nodes.index(where: {$0.prefix == prefix}) {
                    let result = nodes[index].insert(element, with: &prefixes)
                    nodesMap[hashValue] = nodes
                    return result
                } else {
                    var node = _TrieNode<Element>(prefix: prefix)
                    let result = node.insert(element, with: &prefixes)
                    nodes.append(node)
                    nodesMap[hashValue] = nodes
                    return result
                }
            ...
            if let index = elements.index(of: element) {
                ...
                let isInserted = oldElement != element
                if isInserted { elements.append(element) }
                return (isInserted, oldElement)
            } else {
        ...
        ) -> Element?
    {
        if !prefixes.isEmpty {
            ...
            if var nodes = nodesMap[hashValue] {
                if let index = nodes.index(where: {$0.prefix == prefix}) {
                    let removedElement = nodes[index].remove(element, with: &prefixes)
